...
pursuit_model <- function(params, data, control) {
  ...
  # if(verbose) print(m)
  #want = list(perf=diag(pm_w), matrix=pm_w, comp_score=comp_score)
  xslFit(perf = as.numeric(resp_prob), matrix = lexicon + 1e-12, traj = traj) # other: comp_score=as.numeric(comp_score),
}
...
pursuit <- function(gamma, threshold, lambda) {
  xslMod(
    name = "pursuit",
