...
tmp <- xsl_run(guess_and_test(f = .7, sa = 0.9), data = xsl_datasets[1:3]) # ToDo: is this what I want?

xsl_fit(decay(C = .98), data = ex, lower = .8, upper = 1)
xsl_fit(decay(C = .98), data = xsl_datasets[1:3], lower = .8, upper = 1, control = xsl_control(reps = 3))

xsl_fit(pursuit(C = .98), data = ex, lower = .8, upper = 1)

xsl_fit(guess_and_test(f = .7, sa = 0.9), data = ex, lower = c(0.1, 0.1), upper = c(0.1, 0.99),
        control = xsl_control(n_sim = 50))


xsl_fit(PBV(alpha = .5, alpha_increase = .1), data = ex, lower = c(0,0), upper = c(1,1))


xsl_run(kachergis(X = .1, C = 1, B = .98), data = xsl_datasets[1:3])
xsl_run(kachergis(X = .1, C = 1, B = .98), data = xsl_datasets[1:3], control = xsl_control(reps = 3))
