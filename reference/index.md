# Package index

## Models

Cross-situational word learning models

- [`baseline()`](https://www.kachergis.com/XSLmodels/reference/baseline.md)
  : Baseline model
- [`decay()`](https://www.kachergis.com/XSLmodels/reference/decay.md) :
  Decay model
- [`uncfam()`](https://www.kachergis.com/XSLmodels/reference/uncfam.md)
  : Kachergis 2012
- [`uncfam_attention()`](https://www.kachergis.com/XSLmodels/reference/uncfam_attention.md)
  : Biased associative model with attention scaled to trial uncertainty
- [`uncfam_predictive()`](https://www.kachergis.com/XSLmodels/reference/uncfam_predictive.md)
  : Predictive biased associative model (item-level prediction error)
- [`uncfam_sampling()`](https://www.kachergis.com/XSLmodels/reference/uncfam_sampling.md)
  : Kachergis 2012
- [`multi_sampling()`](https://www.kachergis.com/XSLmodels/reference/multi_sampling.md)
  : Multi-sampling associative model
- [`propose_but_verify()`](https://www.kachergis.com/XSLmodels/reference/propose_but_verify.md)
  : Trueswell et al. 2013 propose-but-verify model
- [`pursuit()`](https://www.kachergis.com/XSLmodels/reference/pursuit.md)
  : Stevens et al. 2014 pursuit model
- [`fazly()`](https://www.kachergis.com/XSLmodels/reference/fazly.md) :
  Fazly et al. 2010 probablistic associative model
- [`guess_and_test()`](https://www.kachergis.com/XSLmodels/reference/guess_and_test.md)
  : Guess and test model
- [`rescorla_wagner()`](https://www.kachergis.com/XSLmodels/reference/rescorla_wagner.md)
  : Rescorla-Wagner (1972) error-driven associative model
- [`tilles()`](https://www.kachergis.com/XSLmodels/reference/tilles.md)
  : Tilles and Fontanari (2013) reinforcement model
- [`bayesian_decay()`](https://www.kachergis.com/XSLmodels/reference/bayesian_decay.md)
  : Bayesian decay model
- [`kalman_filter()`](https://www.kachergis.com/XSLmodels/reference/kalman_filter.md)
  : Kalman filter associative model
- [`softmax_rl()`](https://www.kachergis.com/XSLmodels/reference/softmax_rl.md)
  : Softmax reinforcement learning model
- [`fgt2009()`](https://www.kachergis.com/XSLmodels/reference/fgt2009.md)
  : Frank, Goodman & Tenenbaum (2009) intentional Bayesian word-learning
  model
- [`fgt2009_rsa()`](https://www.kachergis.com/XSLmodels/reference/fgt2009_rsa.md)
  : Frank, Goodman & Tenenbaum word-learning model with pragmatic (RSA)
  reasoning

## Running and fitting models

- [`xsl_run()`](https://www.kachergis.com/XSLmodels/reference/xsl_run.md)
  : Run XSL model
- [`xsl_fit()`](https://www.kachergis.com/XSLmodels/reference/xsl_fit.md)
  : Fit XSL model using differential evolution
- [`fgt2009_sweep_alpha()`](https://www.kachergis.com/XSLmodels/reference/fgt2009_sweep_alpha.md)
  : Sweep the lexicon-size prior alpha for an FGT model
- [`update_params()`](https://www.kachergis.com/XSLmodels/reference/update_params.md)
  : Update parameters of xslMod
- [`get_group_model_fit()`](https://www.kachergis.com/XSLmodels/reference/get_group_model_fit.md)
  : Get group model fit for a specific model
- [`get_crossvalidated_model_fit()`](https://www.kachergis.com/XSLmodels/reference/get_crossvalidated_model_fit.md)
  : Get cross-validated model fit
- [`cross_validated_group_fits()`](https://www.kachergis.com/XSLmodels/reference/cross_validated_group_fits.md)
  : Perform Cross-Validation on Group Fits of a Model
- [`get_train_test_split()`](https://www.kachergis.com/XSLmodels/reference/get_train_test_split.md)
  : Split Data into Training and Testing Sets Based on Indices

## Datasets and data structures

- [`xsl_datasets`](https://www.kachergis.com/XSLmodels/reference/xsl_datasets.md)
  : Cross-situational word learning experiment data
- [`kachergis2012_highlighting`](https://www.kachergis.com/XSLmodels/reference/kachergis2012_highlighting.md)
  : Kachergis (2012) highlighting experiment data
- [`kachergis_initial_accuracy`](https://www.kachergis.com/XSLmodels/reference/kachergis_initial_accuracy.md)
  : Kachergis, Grimmick, & Gureckis initial accuracy experiment data
- [`rollins_corpus`](https://www.kachergis.com/XSLmodels/reference/rollins_corpus.md)
  : CHILDES/Rollins naturalistic word-learning corpus (Frank et al.
  2009)
- [`fm_corpus`](https://www.kachergis.com/XSLmodels/reference/fm_corpus.md)
  : Frank, Tenenbaum & Fernald naturalistic word-learning corpus
- [`xslData()`](https://www.kachergis.com/XSLmodels/reference/xslData-class.md)
  [`new_xslData()`](https://www.kachergis.com/XSLmodels/reference/xslData-class.md)
  : xslData S3 class
- [`new_xslFit()`](https://www.kachergis.com/XSLmodels/reference/xslMod-class.md)
  [`xslMod()`](https://www.kachergis.com/XSLmodels/reference/xslMod-class.md)
  [`new_xslMod()`](https://www.kachergis.com/XSLmodels/reference/xslMod-class.md)
  : Constructor for xsl_model S3 class
- [`xslControl()`](https://www.kachergis.com/XSLmodels/reference/xslControl-class.md)
  [`new_xslControl()`](https://www.kachergis.com/XSLmodels/reference/xslControl-class.md)
  : xslControl S3 class
- [`xslFit()`](https://www.kachergis.com/XSLmodels/reference/xslFit-class.md)
  : xslFit S3 class
- [`get_example_ambiguous_condition()`](https://www.kachergis.com/XSLmodels/reference/get_example_ambiguous_condition.md)
  : Get example ambiguous condition
- [`get_example_unambiguous_condition()`](https://www.kachergis.com/XSLmodels/reference/get_example_unambiguous_condition.md)
  : Get example unambiguous condition
- [`show_datasets()`](https://www.kachergis.com/XSLmodels/reference/show_datasets.md)
  : Show available datasets in the package
- [`show_models()`](https://www.kachergis.com/XSLmodels/reference/show_models.md)
  : Show available models in the package
- [`create_cooc_matrix()`](https://www.kachergis.com/XSLmodels/reference/create_cooc_matrix.md)
  : Creates co-occurrence matrix from training trials

## Evaluating model performance

- [`get_perf()`](https://www.kachergis.com/XSLmodels/reference/get_perf.md)
  : Calculate Luce choice (proportion correct) for each item in a model
  knowledge matrix
- [`mafc_test()`](https://www.kachergis.com/XSLmodels/reference/mafc_test.md)
  : Evaluate m-alternative forced choice test
- [`predict_referent()`](https://www.kachergis.com/XSLmodels/reference/predict_referent.md)
  : Predict referent selection for a single trial from an association
  matrix
- [`rsa_listener()`](https://www.kachergis.com/XSLmodels/reference/rsa_listener.md)
  [`rsa_speaker()`](https://www.kachergis.com/XSLmodels/reference/rsa_listener.md)
  : Rational Speech Act pragmatic listener and speaker
- [`get_tp()`](https://www.kachergis.com/XSLmodels/reference/get_tp.md)
  : Get true positives (TP), given a knowledge matrix and a
  gold-standard lexicon
- [`get_fscore()`](https://www.kachergis.com/XSLmodels/reference/get_fscore.md)
  : Calculate F-score, precision, recall, and specificity for a
  knowledge matrix at a given threshold
- [`get_roc()`](https://www.kachergis.com/XSLmodels/reference/get_roc.md)
  [`plot_roc()`](https://www.kachergis.com/XSLmodels/reference/get_roc.md)
  [`get_roc_max()`](https://www.kachergis.com/XSLmodels/reference/get_roc.md)
  : Calculate receiver operating characteristic (ROC) scores for a model
  association Matrix
- [`plot_training_trials()`](https://www.kachergis.com/XSLmodels/reference/plot_training_trials.md)
  : Create Animated Plot of Co-Occurrence Matrix from Training Trials
